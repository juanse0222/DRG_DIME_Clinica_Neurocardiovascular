...
data_egresos <- import("data_egresos_2024.rda")
data_enero_2025 <- import("")
...
data_egresos_2 <- data_egresos_2 %>% 
 ...
 # select(-c(x3, x18))
data_egresos_2 <- data_egresos_2 %>% 
  mutate(cruce = str_c(documento, mes_ingreso, .sep = "")) 
...
data_sca_2 <- data_sca %>% 
  ...
  mutate(fecha_de_ingreso = as.Date(fecha_de_ingreso)) %>% 
  mutate(cruce = str_c(cedula, mes_sca, .sep = ""))
...
data_acv_2 <- data_acv %>% 
  ...
  mutate(cruce = str_c(cedula, mes_acv, .sep = "")) %>% 
...
data_icc_2 <- data_icc %>% 
  ...
  mutate(cruce = str_c(numero_de_identificacion, mes_icc, .sep = ""))
...
###################################################################################################################
# Unión de base SCA
data_egresos_sca <- left_join(x = data_egresos_2, select(data_sca_2, cruce,
...
data_egresos_acv <- left_join(x = data_egresos_2, 
                              select(data_acv_2, cruce, clasificacion_acv, 
                                     mes_acv, fecha_egreso, fecha_ingreso), 
                              by = "cruce")


data_egresos_acv_2 <- data_egresos_acv %>% 
  filter(!is.na(mes_acv)) %>% 
  distinct(documento, numero_de_ingreso, fecha_ingreso.y, fecha_egreso, fecha_de_egreso,
           .keep_all = T) 
...
data_egresos_acv_2 <- data_egresos_acv_2 %>% 
  ...
  mutate(departamento_filtro = paste(departamento_de_ingreso, departamento_actual, sep = " ")) %>% 
  select(-c(dif_dias, fecha_egreso))
...
data_egresos_icc <- left_join(x = data_egresos_2, select(data_icc_2, cruce, 
...
data_grd_6 <- left_join(data_grd_5, select(data_ordenes_2.3, ingreso, pacientes),
                        by = c("numero_de_ingreso" = "ingreso"))
...
data_costo <- read_excel("C:/Users/jshurtado/OneDrive - DIME/Documentos/Documentos Juan Sebastian Hurtado Z/9. CACI 2023/Analisis costos GRD 2024/costo_general_2024.xlsx")
...
data_grd_5.1 <- left_join(data_grd_5, select(data_ordenes_4, ingreso, cuenta, 
                                             departamento_cargue),
                          by = c("numero_de_ingreso" = "ingreso"))
...
data_grd_5 <- data_grd_5 %>% 
  mutate(numero_de_ingreso = as.character(numero_de_ingreso))


data_mmto_4 <- left_join(data_mmto_3, select(data_grd_5, numero_de_ingreso, caci),
                         by = c("ingreso" = "numero_de_ingreso"))
...
data_mmto_5 <- data_mmto_4_1 %>% 
  mutate(mes = month(fecha_adm, label = T)) %>% 
  mutate(costo_2 = (gsub("//,", "", gsub(".", ".", costo, fixed = TRUE))),
...
data_mmto_final <- data_mmto_5 %>% 
  ...
  select(departamento_cargue, fecha_registro, nombres, id, identificacion, 
         ...
         mes_factura, caci, cuenta, costo_4, valor_cargo_tarifario, profesional_asignado) %>% 
...
data_costo_total_2.1 <- data_costo_total_2 %>% 
  mutate(venta = (gsub("//,", "", gsub(".", ".", valor_cargo_tarifario, fixed = TRUE))),
         venta_2 = as.numeric(venta),
         venta_3 = (gsub("//.", "", valor_cargo_tarifario)),
         venta_3 = (gsub(",", ".", venta_3)),
         venta_3 = as.numeric(venta_3),
         venta_4 = case_when(is.na(venta_2) ~ venta_3,
                             TRUE ~ venta_2),
         venta_4 = as.numeric(venta_4)) %>% 
  select(-c(venta, venta_2, venta_3)) %>% 
  rename("venta" = venta_4)
...
export(data_costo_total_3, "data_costo_total_3.rds")
export(data_costo_total_3, "data_costo_total_3.xlsx")
export(data_grd_2, "data_grd_2.rda")

#############################################################################################################
### Total costo CACI mes
data_costo_total_3 <- import("data_costo_total_2.rds")
...
data_costo_total_4 %>% 
  ...
  summarise(mediana = median(costo),
            iqi = quantile(costo, 0.25),
            iqs = quantile(costo, 0.75),
            pacientes = n_distinct(identificacion))
...
data_costo_total_5 %>% 
  ...
  align(align = "center", j = c(2:12), part = "all") %>% 
  bold(i = 1, bold = TRUE, part = "header") %>% 
  bold(i = 13, bold = TRUE, part = "body") %>% 
  merge_at(i = 1:2, j =1, part = "header") %>% 
  merge_at(i = 1:2, j =14, part = "header") %>% 
 # flextable::color(color = "white" ,i = 13, j = c(3,6,9,12), part = "body") %>% 
  flextable::set_caption(caption = as_paragraph(
    as_chunk("Tabla costo medio paciente por CACI, DIME, año 2024", 
             props = fp_text_default(bold = TRUE, font.size = 14)))) %>% 
  footnote(i =2, j=c(3,6,9,12), part = "header", value = as_paragraph(value ="Costo medio corresponde a la mediana de los costos paciente"))


### tabla para exportar costo medio paciente
tabla_costo_medio_2024 <-  data_costo_total_5 %>% 
  mutate(across(,replace_na,0)) %>% 
  bind_rows(summarize(., mes_cargue = "Total", 
                      pacientes_acv = sum(pacientes_acv),
                      mediana_acv = median(mediana_acv), 
                      ...
                      costo_orden_txc = sum(costo_orden_txc),
                      Total = median(Total))) %>% 
  export("tabla costo medio paciente 2024.xlsx")
...
tabla_rentabilidad_2024 <- data_costo_total_6  %>%
  flextable()  %>% 
  add_header_row( top = TRUE,   # La nueva cabecera va encima de la fila de cabecera existente
                  values = c("Mes",     # Valores de cabecera para cada columna a continuación
                             "ACV", "", "",
  ...
  # flextable::color(color = "white" ,i = 13, j = c(3,6,9,12), part = "body") %>% 
  flextable::set_caption(caption = as_paragraph(
    as_chunk("Rentabilidad por mes de los GRD/CACI, DIME, año 2024", 
             props = fp_text_default(bold = TRUE, font.size = 14)))) %>% 
  footnote(i =2, j=c(3,6,9,12), part = "header", value = as_paragraph(value ="Costo medio corresponde a la mediana de los costos paciente"))

tabla_rentabilidad_2024 
#############################################################################################
